...
type LoadedImage struct {
	Revision        uint32
	_               uint32 // 4
	ParentHandle    uint64 // 8
	SystemTable     uint64 // 16
	DeviceHandle    uint64 // 24
	FilePath        uint64 // 32
	_               uint64 // 40
	LoadOptionsSize uint32 // 48
	_               uint32 // 52
	LoadOptions     uint64 // 56
	ImageBase       uint64 // 64
	ImageSize       uint64 // 72
	ImageCodeType   uint64 // 80
	ImageDataType   uint64 // 88
	Unload          uint64 // 96
}
...
func (s *BootServices) LoadImageHandle(imageHandle uint64) (image *LoadedImage, addr uint64, err error) {
	...
	if image.Revision != EFI_LOADED_IMAGE_PROTOCOL_REVISION {
		return nil, 0, errors.New("invalid protocol revision")
	}
...
func (s *Services) Root() (root *FS, err error) {
	...
	if root.image, _, err = s.Boot.LoadImageHandle(s.imageHandle); err != nil {
		return
	}
