...
// CallService calls an UEFI service. The first argument is a pointer
// to the table entry - not to the function to be called.
...
// CallServiceTable takes a point to a UEFI (PE/COFF) table and an index, and 
// calls the function.
func CallServiceTable(tablePtr uintptr, idx int, args []uint64) (status uint64) {
	mux.Lock()
	defer mux.Unlock()

	return callFn(uint64(tablePtr+ 24 + uintptr(8*idx)), len(args), args)
}
